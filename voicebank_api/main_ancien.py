...
SQL: SELECT id_client, nom, prenom, pays_residence, solde_fcfa, banque FROM voicebank.clients WHERE est_diaspora = 'Oui' AND pays_residence = 'France';

Question: "credits en defaut"
SQL: SELECT c.id_client, c.nom, c.prenom, c.banque, cr.type_credit, cr.montant_fcfa, cr.statut_credit FROM voicebank.credits cr JOIN voicebank.clients c ON cr.id_client = c.id_client WHERE cr.statut_credit = 'Défaut';

Question: "transactions frauduleuses"
SQL: SELECT id_transaction, id_client, banque, montant_fcfa, date_transaction FROM voicebank.transactions WHERE est_frauduleuse = 'Oui' ORDER BY montant_fcfa DESC;
...
def texte_vers_sql(question: str) -> tuple:
    ...
    if not gemini_client:
        raise HTTPException(503, "Gemini non disponible. Verifie GEMINI_API_KEY dans .env")
    ...
    response = gemini_client.models.generate_content(
        ...
        model="gemini-2.5-flash",  # plus rapide, moins cher, mais moins performant
        contents=prompt,
    )
...
async def transcrire_audio(fichier: UploadFile = File(...)):
    ...
    try:
        ...
        return {"status": "ok", "texte": result["text"].strip()}
    except Exception as e:
        raise HTTPException(500, str(e))
    finally:
